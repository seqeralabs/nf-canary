...
def render_html(combined_report: Dict[str, Any], template_str: Optional[str] = None) -> str:
    """
    ...
    Args:
        ...
        template_str: The HTML template string (pre-loaded for robustness).
                     If None, will be loaded at call time (slower, for backward compatibility).
...
def load_template() -> str:
    """Load the HTML template from assets/templates directory.
    ...
    template_path = Path(__file__).parent.parent / "assets" / "templates" / "fusion_report_template.html"
    try:
        ...
        raise IOError(f"Cannot read template file {template_path}: {str(e)}")
